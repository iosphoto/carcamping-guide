...
Nothing pending. **Live version is v33** — all committed and pushed. Verify the latest deploy rendered (Netlify auto-builds on push, ~1 min), then pick up the next task below.
...
- **v33:** **Scoreboard-as-navigation + nav declutter.** `#logged-in-nav` decluttered — removed the "Profile" button and the name label; kept `+ Post` + `Sign out` (and the browse tabs). "Feed" renamed **News** (nav label + default content). The profile **scoreboard IS the navigation**: the 6 stat tiles are clickable (`switchProfileSection`, tiles carry `#pub-tile-*`) and swap the content area below; **News** (the journal feed) is the default with a `📰 News` / `← News` toggle; the two itinerary tiles (Trips Planned / Nights Camped) open those trip lists. The duplicate `.profile-tabs` strip was removed (one navigation now). Added the **Hot Gear** list; dropped the off-scoreboard **Boondocking / Hot Views / Bookings** profile panels (their posts still surface in News). Verified live: nav declutter, real tile clicks swap+highlight, News default/back, no console errors. **Note:** with the Profile button gone, the only path back to your profile from the Browse view is browser Back or reload (both re-land on profile) — flag if a persistent home affordance is wanted.
